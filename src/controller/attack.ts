...
export const attack = async (game: GameSessionRepository, ws: WebSocket, message: Message) => {
  ...
  try {
    ...
    const user1 = await game.users.getUserById(indexPlayer);
    ...
    if (!isSinglePlayer) {
      ws2 = (await game.users.getUserById(idPlayer2))?.ws;
    }
    ...
    if (isWin) {
      logger(MessageType.finish, `Player ${setColor(user1.name, 'blue')} won the game.`);
      return;
    }

    logger(message.type, `Player ${setColor(user1.name, 'blue')} attacked the position x:${setColor(x, 'yellow')}, y:${setColor(y, 'yellow')}. Attack status: ${setColor(result.status, 'green')}`);
    ...
    if (!isSinglePlayer || isHit) {
      ...
      while (continueAttack) {
        ...
        logger(message.type, `${setColor('Player bot', 'blue')} attacked the position x:${setColor(x, 'yellow')}, y:${setColor(y, 'yellow')}. Attack status: ${setColor(result.status, 'green')}`);
      ...
      const isWin = await handleWin(game, coordinates, singlePlayerBotId, ws);
        if (isWin) {
          logger(MessageType.finish, `Bot player won the game.`);
